...
# ---------- Testes Gerais de Validação e Formatação ----------

def test_nome_title_case_e_strip():
    """Verifica que o nome é stripado e convertido para Title Case."""
    est = Estagiario.criar("   joão silva    ", 10)
    assert est.nome == "João Silva"
...
def test_horas_float_aceito_como_inteiro():
    ...
    est = Estagiario.criar("João Silva", 100.0)
...
def test_horas_float_nao_inteiro_lanca_erro():
    ...
    with pytest.raises(ValueError, match="Horas trabalhadas deve ser inteiro ou float equivalente a inteiro"):
        Estagiario.criar("João Silva", 10.5)
...
def test_estagiario_salario_total_com_ferias():
    ...
    estagiario = Estagiario.criar("João Silva", horas_trabalhadas, ferias=True)
...
def test_efetivo_salario_sem_horas_extras():
    ...
    efetivo = Efetivo.criar("Maria Souza", horas)
...
def test_efetivo_salario_com_horas_extras():
    ...
    efetivo = Efetivo.criar("Maria Souza", total_horas)
...
def test_efetivo_salario_total_com_ferias():
    efetivo = Efetivo.criar("Maria Souza", 100, ferias=True)
...
def test_efetivo_to_dict():
    efetivo = Efetivo.criar("Maria Souza", 150, ferias=False)
...
def test_vendedor_salario_sem_bonus_vendas():
    ...
    vendedor = Vendedor.criar("Carlos Lima", horas_trabalhadas, vendas=vendas)
...
def test_vendedor_salario_total_com_ferias():
    vendedor = Vendedor.criar("Carlos Lima", 50, vendas=5000.0, ferias=True)
...
def test_vendedor_vendas_negativas():
    with pytest.raises(ValueError, match="Valor de vendas não pode ser negativo"):
        Vendedor.criar("Carlos Lima", 40, vendas=-1000.0)
...
def test_vendedor_vendas_invalida_string():
    with pytest.raises(ValueError, match="Valor de vendas deve ser numérico ou string numérica"):
        Vendedor.criar("Carlos Lima", 40, vendas="abc")

def test_vendedor_to_dict():
    vendedor = Vendedor.criar("Carlos Lima", 80, vendas=10000.0)
    dados = vendedor.to_dict()
    assert dados['nome'] == "Carlos Lima"
...
# ---------- Testes de Freelancer ----------

def test_freelancer_salario_sem_bonus_horas():
    projetos = 5
    horas_trabalhadas = 80  # abaixo do limite de bônus
    freelancer = Freelancer.criar("Ana Costa", horas_trabalhadas, projetos=projetos)
...
def test_freelancer_salario_total_com_ferias():
    freelancer = Freelancer.criar("Ana Costa", 120, projetos=3, ferias=True)
...
# ---------- Testes de Fábrica de Funcionários ----------
...
def test_fabrica_cria_tipos_nativos_corretamente():
    ...
    est = FabricaFuncionario.criar("estagiario", "João Silva", 50)
    assert isinstance(est, Estagiario)
    assert est.horas == 50

    efet = FabricaFuncionario.criar("efetivo", "Maria Souza", 200)
    assert isinstance(efet, Efetivo)
    assert efet.horas == 200

    vend = FabricaFuncionario.criar("vendedor", "Carlos Lima", 40, vendas=3000)
    assert isinstance(vend, Vendedor)
    assert vend.vendas == Decimal('3000')

    freel = FabricaFuncionario.criar("freelancer", "Ana Costa", 100, projetos=2)
...
def test_relatorio_texto_formatacao_efetivo():
    efet = Efetivo.criar("Maria Souza", 100, ferias=False)
    rel = RelatorioTexto().gerar(efet)
    assert "RELATÓRIO SALARIAL" in rel
    assert "Nome: Maria Souza" in rel
...
def test_relatorio_texto_formatacao_vendedor():
    vend = Vendedor.criar("Carlos Lima", 80, vendas=5000.0)
...
def test_relatorio_texto_formatacao_freelancer():
    freel = Freelancer.criar("Ana Costa", 120, projetos=3)
    rel = RelatorioTexto().gerar(freel)
    assert "RELATÓRIO SALARIAL" in rel
    assert "Nome: Ana Costa" in rel
...
def test_relatorio_json_efetivo_valido():
    efet = Efetivo.criar("Maria Souza", 150, ferias=True)
...
def test_relatorio_json_vendedor_valido():
    vend = Vendedor.criar("Carlos Lima", 50, vendas=2000.0, ferias=False)
...
def test_relatorio_json_freelancer_valido():
    freel = Freelancer.criar("Ana Costa", 90, projetos=4, ferias=False)
